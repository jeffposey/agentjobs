...
**Two tracked files are what to read instead.** A database beside a server is the right
home for a backlog and the wrong shop window for a public repository, so the open work
reaches the repository twice, once each way.

[docs/backlog.md](docs/backlog.md) is the projection: every ready and in-progress task,
in the order the queue hands it out, with what each one is waiting on. It is generated by
`scripts/export_roadmap.py`, nothing reads it back, and it is regenerated when the
`roadmap` playbook runs rather than on every task change, so it can run behind the store
between passes. Regenerate it rather than editing it:
