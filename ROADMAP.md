...
- **task-428** — Tested and working on Linux: the CLI, the server, dispatch and the gate on a machine that is not Windows

  AgentJobs has only ever run on one Windows machine, and every POSIX branch in its process handling is unexecuted. Run it for real on Linux, file each breakage as a child, and end with a gate that runs there.
...
- **task-429** — A first-run walkthrough that somebody other than the author has followed from a clean machine
  ...
  Write the guide a stranger follows from nothing to a first dispatched task, then have a person who did not build AgentJobs follow it on a clean machine. Every place they got stuck becomes a fix or a task.

  *needs task-127-publish-and-invoke-without-a-venv · needs task-128-onboarding-from-inside-a-session*

- **task-430** — Give a user who is not the author somewhere to report what broke

  AgentJobs files its own issues into its own store, which only its owner can reach. Decide and publish where an outside user reports a problem, and make the product point them there when it fails.

- **task-431** — State what AgentJobs promises a user about their data: where it lives, what leaves the machine, and how to back it up

  A user's task records live in a SQLite database beside the server, outside every checkout, and nothing tells them so. Publish a short statement of where their data is, what ever leaves their machine, and how to back it up and restore it.
