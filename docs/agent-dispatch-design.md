...
| How a session is woken | `ClaudeSessionNudger`, task-417 entry 8's verified sequence: `stop`, observe no pid in `agents --json --all`, then `--bg --resume <full uuid>` with the message on stdin and no other flag, so the saved runner, model and posture come back. A launcher that reports a copy is `not_applied`, and the copy is stopped |
