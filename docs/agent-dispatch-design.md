...
| What a red retry is called | `judge_second_red` (task-526). A failure line -- nodeid and assertion text -- printed by both attempts makes it `deterministic_in_context`: chance does not repeat verbatim, so the branch or the machine is the cause. A flake guess whose retry went red on a different test stays `flaky_test` and says so. Recorded as a `finish_gate_red_twice` phase, which `agentjobs execution failures` counts as `stopped`, and in the escalation entry's `data.gate_red_twice` |
| What a red pytest escalation hands over | `flake_register_entry`: the escalation body and its `ball_prompt` name `docs/flake-register.md` and carry one ready-to-paste row per failing test, with each attempt's gate width (as `gate_slots.note` printed it) and log path |
...
**Defects found and fixed by it.** Each fix has a test that fails without it:
