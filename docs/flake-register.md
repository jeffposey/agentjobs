...
| 21 | `tests/test_execution_processes.py::TestAdmissionAcrossProcesses::test_identical_task_ids_in_different_projects_never_share_ownership` | bare `AssertionError` at line 126 (no assertion text) | not named | unknown | **open** -- seen by finish `fin_294daacb` (task-594) on `c4e1e627`, attempt 1, `-n 26`, alone on the machine; green on the `--from pytest` retry |
| 22 | `frontend/src/components/ReviewDocuments.test.tsx` > renders a Markdown deliverable with where it was read from | `findByRole("heading", { name: "The design" })` timed out; the test took 1561 ms against the 1000 ms default | waiting for the heading covers a fetch, a Suspense boundary, the lazy `import("./MarkdownDocument")` (react-markdown and remark-gfm; 143 ms idle) and a Markdown parse. That fits comfortably when the file runs alone (~450 ms) but not under the full vitest run's own parallel workers. With the import preloaded, the same test measured 840, 887 and 1002 ms across three full runs, so the preload alone did not fix it | test premise | **fixed** on task-594's branch before it merged: the import is awaited in `beforeAll`, and waits that render a document use a named 5 s timeout (`RENDERED`), because the test checks what renders, not how fast |
