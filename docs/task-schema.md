...
| `kind` | enum | Optional: `design` · `implementation`. **Absent means `implementation`**, so every record written before the field existed keeps its meaning and nothing had to be migrated. Any actor may change it through the content patch (`task_update_content`, `PATCH /tasks/{id}`), logged like `category`. That is safe because **it grants nothing**: it changes how a task is shown and how an approval is worded, never what an approval authorises — the handoff's gate does that. `GET /api/tasks?kind=implementation` includes tasks with no kind set. Design in [task-kind-design.md](task-kind-design.md#1-the-discriminator-a-kind-field) (task-592). |
