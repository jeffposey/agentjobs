#!/usr/bin/env python3
"""Deny attempts to write AgentJobs task YAML with general file or shell tools.

A Codex ``PreToolUse`` hook. It reads one JSON event on stdin, decides, and writes one
JSON decision to stdout.

**This is a guardrail, not a security boundary, and the difference matters.** It sees
the tool calls Codex routes through it and nothing else: a hosted or specialised tool
it never observes, a hook the user has disabled or not trusted, an obfuscated script,
a path assembled at runtime from variables, or any process started outside Codex will
all get past it. Its job is to catch the realistic accident -- an agent reaching for
``apply_patch`` because that is the tool it knows -- and turn it into a sentence
naming the managed tools instead. Treating it as enforcement would be a mistake that
the layers in task-118 exist to cover.
...
SHELL_TOOLS = {"bash", "shell", "powershell", "pwsh", "cmd", "run_command", "local_shell"}
...
class Decision:
    """What the hook tells Codex to do."""
...
def managed_directories(env: Optional[Dict[str, str]] = None) -> List[Path]:
    ...
    hook must work when the package is not importable from the Codex process, and it
...
def _registry_roots(registry: Path) -> List[Path]:
    ...
    YAML parser the Codex environment may not have.
    """
    try:
...
def evaluate(event: Dict[str, Any], directories: Sequence[Path], cwd: Path) -> Dict[str, Any]:
    ...
    if lowered in FILE_WRITE_TOOLS:
        targets = [
            str(value)
        ...
        return {"permission_decision": Decision.ALLOW}

    if lowered in SHELL_TOOLS or "command" in arguments:
...
def _deny(paths: Iterable[str], directory: Optional[Path], reason: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "permission_decision_reason": _denial(paths, directory or Path("."), reason),
    }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    Any internal failure allows the call. A guardrail that breaks tool use when it
    hits an event shape it does not recognise is worse than the accident it prevents,
    and the layers in task-118 catch what slips through.
    """
    del argv
    try:
        event = json.loads(sys.stdin.read() or "{}")
    except (ValueError, OSError):
        print(json.dumps({"permission_decision": Decision.ALLOW}))
        return 0
    try:
        ...
        decision = {"permission_decision": Decision.ALLOW}
