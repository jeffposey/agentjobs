...
**DO NOT:**
- ❌ Work in `/mnt/c/projects/tradinator`
- ❌ Copy agentjobs files into tradinator
- ❌ Create `tradinator/src/agentjobs/` directory
- ❌ Make any changes to tradinator repo (yet - that's Phase 5)

**This prompt is stored in tradinator but the WORK is in agentjobs.**
...
- Parses existing Markdown task files (Tradinator format)
...
**Target Use Case**: Migrate 30+ Tradinator tasks from `ops/tasks/*.md` to structured YAML.
...
```bash
cd /mnt/c/projects/agentjobs
git status  # Verify you're in agentjobs, NOT tradinator
```
...
# Create sample test data (optional - use Tradinator files later)
...
Features:
- Parse Markdown task files (Tradinator format)
...
**Phase 5 will use this tool** to migrate Tradinator's 30+ tasks.
