...
reports its own pid, sleeps, forks, exits with ``os._exit``, reads its working directory
or is killed by the test is about a process's lifetime and keeps its real spawn. Every
file that installs this says so beside the install, which is the split task-525's
acceptance asks the test files to state.
"""
